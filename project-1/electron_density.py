...
def compute_electron_density(temperature, atom, fraction=0.5):
    T = temperature
    ...
    N_e = fraction * 2 * U_rplus1 / U_r * saha_const * np.exp(-chi_ion / (const.k_B * T))

    return N_e

h = Atom('H_atom.txt')
T = 9500 * units.K 
N_e = compute_electron_density(T, h)
